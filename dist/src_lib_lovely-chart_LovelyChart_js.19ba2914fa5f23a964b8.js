...
function createAxes(context, data, plotSize, colors) {
  ...
  function _drawYAxisScaled(state, projection, scaleLevel, yMin, yMax) {
    let opacity = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1;
    let colorKey = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : null;
    let isSecondary = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
...
function createHeader(container, title) {
  let zoomOutLabel = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'Zoom out';
  let zoomOutCallback = arguments.length > 3 ? arguments[3] : undefined;
  let _element;
  let _titleElement;
  let _zoomOutElement;
  let _captionElement;
...
function create(container, originalData) {
  ...
  function _onStateUpdate(state) {
    ...
    const visibilities = datasets.map(_ref => {
      let {
        key
      } = _ref;
...
function createMinimap(container, data, colors, rangeCallback) {
  ...
  function _isStateChanged(newState) {
    ...
    if (datasets.some(_ref => {
      let {
        key
      } = _ref;
      return _state[`opacity#${key}`] !== newState[`opacity#${key}`];
    })) {
      return true;
    }
  ...
  function _drawDatasets() {
    ...
    const visibilities = datasets.map(_ref2 => {
      let {
        key
      } = _ref2;
      return _state[`opacity#${key}`];
    });
  ...
  function _onSliderDrag(moveEvent, captureEvent, _ref3) {
    let {
      dragOffsetX
    } = _ref3;
    const minX1 = 0;
  ...
  function _onLeftEarDrag(moveEvent, captureEvent, _ref4) {
    let {
      dragOffsetX
    } = _ref4;
    const minX1 = 0;
  ...
  function _onRightEarDrag(moveEvent, captureEvent, _ref5) {
    let {
      dragOffsetX
    } = _ref5;
...
function createStateManager(data, viewportSize, callback) {
  ...
  function update() {
    ...
    let noTransition = arguments.length > 1 ? arguments[1] : undefined;
    ...
    if (!noTransition) {
      _transitionConfig.forEach(_ref => {
        let {
          prop,
          duration,
          options
  ...
  function _buildTransitionConfig() {
    ...
    const datasetVisibilities = data.datasets.map(_ref2 => {
      let {
        key
      } = _ref2;
      return `opacity#${key} 300`;
    });
...
function calculateYRangesForGroup(data, labelFromIndex, labelToIndex, prevState, datasets) {
  ...
  } = (0,_utils__WEBPACK_IMPORTED_MODULE_1__.getMaxMin)((0,_utils__WEBPACK_IMPORTED_MODULE_1__.mergeArrays)(datasets.map(_ref5 => {
    let {
      yMax,
      yMin
    } = _ref5;
  ...
  if (labelFromIndex === 0 && labelToIndex === data.xLabels.length - 1) {
    ...
    const filteredValues = datasets.map(_ref6 => {
      let {
        values
      } = _ref6;
      return values;
...
function calculateYRangesStacked(data, filter, labelFromIndex, labelToIndex, prevState) {
  ...
  const filteredValues = filteredDatasets.map(_ref7 => {
    let {
      values
    } = _ref7;
    return values;
...
function createTools(container, data, filterCallback) {
  ...
  function _updateFilter(button) {
    let isLongPress = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
function createTooltip(container, data, plotSize, colors, onZoom, onFocus) {
  ...
  function _selectLabel(isExternal) {
    ...
    const statistics = data.datasets.map((_ref, i) => {
      let {
        key,
        name,
        values,
  ...
  function _drawCircle(_ref4, strokeColor, fillColor) {
    let [xPx, yPx] = _ref4;
  ...
  function _insertNewDataSet(dataSetContainer, _ref5, totalValue) {
    let {
      name,
      key,
      value
  ...
  function _updateDataSet(currentDataSet) {
    ...
    let totalValue = arguments.length > 2 ? arguments[2] : undefined;
  ...
  function _updateDataSets(statistics) {
    ...
    const pointerVector = getPointerVector();
    const finalStatistics = data.isPie ? statistics.filter((_ref6, index) => {
      let {
        value
      } = _ref6;
      return _isPieSectorSelected(statistics, value, totalValue, index, pointerVector);
    }) : statistics;
...
function setupCanvas(container, _ref) {
  let {
    width,
    height
  } = _ref;
...
function preparePoints(data, datasets, range, visibilities, bounds, pieToArea) {
  let values = datasets.map(_ref => {
    let {
      values
    } = _ref;
    return values.slice(range.from, range.to + 1);
  });
...
function getSumsByY(points) {
  return (0,_utils__WEBPACK_IMPORTED_MODULE_0__.sumArrays)(points.map(datasetPoints => datasetPoints.map(_ref2 => {
    let {
      visibleValue
    } = _ref2;
    return visibleValue;
...
function toggleText(element, newText) {
  let className = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
  let inverse = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
...
//# sourceMappingURL=src_lib_lovely-chart_LovelyChart_js.19ba2914fa5f23a964b8.js.map